...
func checkListPointers(t *testing.T, l *List, es []*Node) {
    ...
    if len(es) == 0 {
        if l.head.next != nil && l.head.next != head || l.head.prev != nil && l.head.prev != head {
            t.Errorf("l.head.next = %p, l.head.prev = %p; both should both be nil or %p", l.head.next, l.head.prev, head)
        }
        return
    }
    // len(es) > 0
